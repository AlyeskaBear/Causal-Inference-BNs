# https://paper-submission.shinyapps.io/BNsCausalInference/
...
ex2.Culture<-data.frame(A=character(1),B=character(1))
ex2.Color<-data.frame(Culture=c("A", "B"), Black=character(2), Blue=character(2), Red=character(2))

ex3.Hospital<-data.frame(Male=character(1),Female=character(1))
ex3.Treatment<-data.frame(Medication=character(1), Placebo=character(1))
ex3.Recovery<-data.frame(Hospital=c("A","A","B","B"),
...
# 
#ex2.Culture<-data.frame(Male=c("50/100"),Female=c("50/100"))
#ex2.Color<-data.frame(Culture=c("A", "B"), Black=c("25/50","5/50"), Blue=c("15/50","10/50"), Red=c("10/50","35/50"))
# 
#ex3.Hospital<-data.frame(A=c("40/80"),B=c("40/80"))
...
server<-function(input, output, session){
  ...
  output$BNmodel <- renderUI({
    ...
    else if(input$example=="Color Preference by Culture"){
      HTML("P(Culture,Color)=P(Culture)P(Color|Culture)") # Example 2
    }
    
    else if(input$example=="Efficacy of Medication Treatment"){
      HTML("P(Treatment,Hospital,Recovery)=P(Recovery|Hospital,Treatment)P(Treatment)P(Hospital)") # Example 3
    }
  ...
  output$vars <- renderUI({
    if(input$example=="Sex as a Cause of Pregnancy"){
      ...
      radioButtons("var","Complete the probability table(i.e., the row sum of each row is one) for",selected="culture",
                   list("Culture"="culture","Color"="color") # Example 2
      )
    }
    
    else if(input$example=="Efficacy of Medication Treatment"){
      radioButtons("var","Complete the probability table(i.e., the row sum of each row is one) for",selected="hospital",
                   list("Hospital"="hospital","Treatment"="treatment", "Recovery"="recovery") # Example 3
      )
    }
    
    else if(input$example=="Should I Switch?"){
      radioButtons("var","Complete the probability table(i.e., the row sum of each row is one) for",selected="jen",
  ...
  output$BNInput <- renderRHandsontable({
    ...
    else if ((input$example=="Color Preference by Culture")&(input$var=="culture")){
      if (is.null(ex2.Culture.values$data)==FALSE) {rhandsontable(ex2.Culture.values$data)}
      else {rhandsontable(ex2.Culture)}
    }
    else if ((input$example=="Color Preference by Culture")&(input$var=="color")){
      if (is.null(ex2.Color.values$data)==FALSE) {rhandsontable(ex2.Color.values$data)}
      else {rhandsontable(ex2.Color)}
    }
    
    else if ((input$example=="Efficacy of Medication Treatment")&(input$var=="hospital")){
      if (is.null(ex3.Hospital.values$data)==FALSE) {rhandsontable(ex3.Hospital.values$data)}
      else {rhandsontable(ex3.Hospital)}
    }
  ...
  ex2.Culture.values <- reactiveValues()
  ex2.Color.values <- reactiveValues()
  
  ex3.Hospital.values <- reactiveValues()
  ...
  observeEvent(input$runButton, {
    if ((input$example=="Sex as a Cause of Pregnancy")&(input$var=="sex")){
      ...
      ex2.Culture.values$data <-  hot_to_r(input$BNInput)}
    else if ((input$example=="Color Preference by Culture")&(input$var=="color")){
      ex2.Color.values$data <-  hot_to_r(input$BNInput)}
    
    else if ((input$example=="Efficacy of Medication Treatment")&(input$var=="hospital")){
      ex3.Hospital.values$data <-  hot_to_r(input$BNInput)}
    else if ((input$example=="Efficacy of Medication Treatment")&(input$var=="treatment")){
  ...
  output$BNOutput <- DT::renderDataTable({
    ...
    else if ((input$example=="Color Preference by Culture")&(input$var=="culture")){ex2.Culture.values$data}
    else if ((input$example=="Color Preference by Culture")&(input$var=="color")){ex2.Color.values$data}
    
    else if ((input$example=="Efficacy of Medication Treatment")&(input$var=="hospital")){ex3.Hospital.values$data}
  ...
  observeEvent(input$runButton2, {
    if (input$example=="Sex as a Cause of Pregnancy"){
      ...
      # Propose the causal structure
      # P(Culture,Color)=P(Culture)P(Color|Culture)
      Ex2.DAG <- model2network('[Culture][Color|Culture]')
      
      # Create the probability tables for the nodes of Culture and Color
      # specify different states of a node
      culture <- c("A","B")
      ...
      Culture.value<-as.data.frame(ex2.Culture.values$data)
      Culture.value<-as.matrix(Culture.value) 
      Culture.value<-sapply(Culture.value, function(x) eval(parse(text=x)))
      
      
      # S <- array(dimnames = list(Culture = culture), dim = 2, c(50/100,50/100))
      S <- array(dimnames = list(Culture = culture), dim = 2, Culture.value)
      # the Color node 
      # child node <-- parent node
      ...
      # Propose the causal structure
      # P(Treatment, Hospital, Recovery)=P(Recovery|Hospital,Treatment)P(Treatment)P(Hospital)
      Ex3.DAG <- model2network('[Hospital][Treatment][Recovery|Hospital:Treatment]')
      
      # Create the probability tables for the nodes of Recovery, Treatment and Hospital
      # specify different states of a node
      recovery<- c("Yes","No")
      ...
      # the Hospital node
      # parent node
      Hospital.value<-as.data.frame(ex3.Hospital.values$data)
      Hospital.value<-as.matrix(Hospital.value) 
      Hospital.value<-sapply(Hospital.value, function(x) eval(parse(text=x)))
      
      S <- array(dimnames = list(Hospital = hospital # Male=18+12+7+3=40; Female=2+8+9+21=40
      ), dim = 2, Hospital.value)
  ...
  output$vars2 <- renderUI({
    if(input$example=="Sex as a Cause of Pregnancy"){
      ...
      radioButtons("var2","Postintervention distribution for",selected="Culture",
                   list("Culture"="culture","Color"="color") # Example 2
      )
    }
    
    else if(input$example=="Efficacy of Medication Treatment"){
      radioButtons("var2","Postintervention distribution for", selected="hospital",
                   list("Hospital"="hospital","Treatment"="treatment", "Recovery"="recovery") # Example 3
      )
    }
  ...
  output$postIntC<-renderPlot({
    ...
    else if ((input$example=="Color Preference by Culture")&(input$var2=="culture")){
      bn.fit.barchart(as.bn.fit(Ex2.postInt$result,including.evidence = TRUE)$Culture, main="Culture", xlab="Posterintervention Distribution") # Example 2
    }
    else if ((input$example=="Color Preference by Culture")&(input$var2=="color")){
    ...
    else if ((input$example=="Efficacy of Medication Treatment")&(input$var2=="hospital")){
      bn.fit.barchart(as.bn.fit(Ex3.postInt$result,including.evidence = TRUE)$Hospital, main="Hospital", xlab="Posterintervention Distribution") # Example 3
    }
